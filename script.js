...
const botLiveStatus = document.getElementById('botLiveStatus');
const copyCommandStatus = document.getElementById('copyCommandStatus');
const copyCommandButtons = document.querySelectorAll('[data-copy-command]');

if (botLiveStatus) {
  const isOnline = !!discordUser;
  botLiveStatus.textContent = isOnline ? 'Online (Discord linked)' : 'Ready';
}

if (copyCommandButtons.length) {
  copyCommandButtons.forEach(btn => {
    btn.addEventListener('click', async () => {
      const value = btn.getAttribute('data-copy-command') || '';
      if (!value) return;
      ...
      if (copyCommandStatus) {
        copyCommandStatus.textContent = ok ? ('Copied: ' + value) : 'Copy failed. Select and copy manually.';
        copyCommandStatus.classList.toggle('ok', ok);
      }
      if (ok) playSfx('click');
    });
  });
